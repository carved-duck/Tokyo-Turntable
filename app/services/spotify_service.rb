...
class SpotifyService
  ...
  # Tries known hits in JP→GB→US, then artist→top-tracks JP→GB→US.
  def get_band_sample_preview_url(band_name)
    ...
    # 2) Fallback: look up the artist, then try top-tracks (with re-search logic)
    ["JP", "GB", "US"].each do |market|
      artist_id = search_artist(band_name, market)
      if artist_id
  ...
  # Search for a single track by query in the given market, return preview_url or nil
  def search_track_preview_url(query, market = "JP")
    return nil if query.blank?
    _search_track_preview_url(query, market)
  end

  # Search for an artist by name in the given market, return artist ID or nil
  def search_artist(artist_name, market = "JP")
    return nil if artist_name.blank?
    _search_artist_id(artist_name, market)
  end

  # Given a Spotify artist ID, fetch top‐tracks in the given market and return the first preview_url (or nil)
  ...
  def fetch_with_token(path, query = {})
    ...
    # If our token expired, Spotify returns 401. Fetch a new token and retry once.
    if resp.code == 401
      @access_token = fetch_access_token
      resp = self.class.get(
        path,
  ...
  # Raw search for a track by full query (e.g. "Radiohead Creep") in a given market.
  ...
  # Raw search for artist ID by name in a given market. Returns artist_id (string) or nil.
  ...
  # Given artist_id, fetch top-tracks in a market; return first preview_url or nil.
  # If the returned track list has `preview_url: null`, do NOT re-search each title here—
  # instead, rely on search_track_preview_url for known hits (per get_band_sample_preview_url).
