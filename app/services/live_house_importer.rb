...
class LiveHouseImporter
  ...
  def import_gigs_from_json()
    ...
    gigs_data.each do |gig_attributes|
      ...
      gig_open_time = gig_attrs['open_time'].to_s.strip
      ...
      gig = Gig.find_or_initialize_by(
          venue: gig_attributes[:venue],
          date: gig_attributes[:date],
          open_time: gig_attributes[:open_time]
        )
      if gig.save
      ...
      if gig.save
        puts "  Imported/Updated: #{gig.name}"
      else
